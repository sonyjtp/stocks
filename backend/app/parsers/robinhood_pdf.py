...
def extract_ticker_from_text(text: str) -> Optional[str]:
    ...
    """
    skip = {
        'M', 'C', 'D', 'A', 'B', 'AND', 'OF', 'THE', 'IN', 'AT',
        'COMMON', 'STOCK', 'CORP', 'CORPORATION', 'INC', 'INCORPORATED',
        'LLC', 'LP', 'LTD', 'LIMITED', 'CO', 'COMPANY', 'UNITS',
        'PARTNERSHIP', 'CLASS', 'MARGIN', 'UNSOLICITED', 'CUSIP', 'ADR',
        'DEPOSIT', 'FEE', 'ACH', 'GOLD',
    }
...
def _parse_2019_format(trans_type: str, description: str, date_str: str, numeric_line: str) -> Optional[Dict[str, Any]]:
    activity_date = parse_date(date_str)
    if not activity_date:
        return None
